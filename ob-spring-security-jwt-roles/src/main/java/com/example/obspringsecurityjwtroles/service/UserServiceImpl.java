...
public class UserServiceImpl implements UserDetailsService, UserService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        //get authority carga roles del usuario
    }
    ...
    public User save(UserDto user) {
        ...
        User nUser = user.getUserFromDto(); //generamos user

        if(userRepository.existsByEmail(nUser.getEmail()))
            throw new EmailAlreadyExistsException("Email ocupado");

        nUser.setPassword(bcryptEncoder.encode(user.getPassword())); //reemplazamos contraseña por la cifrada
        ...
        if(nUser.getEmail().split("@")[1].equals("admin.edu")){ //se le asigna rol admin si el email termina en admin.edu
